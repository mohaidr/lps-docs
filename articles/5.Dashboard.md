...
3. **Built-In Configuration**:  
  The dashboard can be enabled, disabled, and configured with the `lps dashboard` command. This flexibility is particularly useful when integrating with DevOps tools or custom workflows.
...
- If TCP/TLS/Sending stay stable and WaitingTime increases:
  → Likely indicates slower backend, but not guaranteed.

**Not Safe To Claim:**
- “WaitingTime = pure server time” (incorrect).
...
Use the `dashboard` command to update the persistent dashboard settings:
...
Each option is optional. Settings that are not provided keep their current values. For example, this changes only the refresh rate:

```bash
lps dashboard --refreshrate 10
```
...
- `-rr`, `--refreshrate <seconds>`: Sets how often metrics are pushed to the dashboard. The value must be greater than `0`.

Run `lps dashboard --help` to display the available options.

The command persists these values in `config/lpsSettings.json` under `LPSAppSettings:Dashboard`. The equivalent JSON structure is:
...
```bash
lps dashboard --builtindashboard false
```
