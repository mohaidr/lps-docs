...
**Description**  
Maintains a stateful counter that advances by `step` from `start` to `reset` (inclusive). When the counter hits the `reset` value, it **wraps back** to `start`.

**Parameters**
- `isGlobal` *(bool, optional, default: false)* – Should the counter be a global counter or per user session.
- `start` *(int, optional, default: 0)* – The value the counter starts with.
- `reset` *(int, optional, default: 100000)* – The value at which the counter will reset to the start value.
...
**Returns**  
- The current counter value (as string). Empty if unexpected error occures

**State & Scope**  
- The counters called inside an iteration are session specific and do not collide with the global counters unless isGlobal is set to true.  
- Using different `counter` names creates distinct counters even with the same `start`/`end`.

**Side‑effects**  
- Stores to `variable` when provided.
- When hits the reset value, the counter resets to `start`.

**Example**
```yaml
variables:
- name: seq
  value: $counter(start=1, reset=3, step=1, counter=myCounter, isGlobal = false, variable=myVar)
