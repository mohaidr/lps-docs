...
**Description**  
The conditional counterpart of `read`. Evaluates a boolean `condition`; when it is **true**, it performs the same read as `read` (from a **file**, **environment variable**, or **another variable**) and stores the result. When it is **false**, it stores `else` if provided, otherwise the variable is left **untouched** (no read happens).
...
**Notes**
- XML namespaces are not currently supported (use non‑namespaced XML, or match by local name).
- External entities are disabled (protects against XXE).
