...
- **Startup Delay**: Time in seconds to wait before the round starts.
...
- **Step-down**: Start high and reduce load to observe recovery behavior.
...
2. **Stage 2** — Waits 5000 ms after stage 1 finishes spawning, then 50 clients arrive 1000 ms apart (spawn window: 49000 ms).
3. **Stage 3** — Starts immediately after stage 2 finishes spawning (no `startupDelay`), then 5 clients arrive 500ms apart.

All 65 clients share the same iterations and run concurrently once spawned.
