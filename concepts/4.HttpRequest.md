...
`before` is also available at the **iteration** level (runs once when the iteration starts, before the iteration's `skipIf`). See the [Before Expressions article](https://github.com/mohaidr/lps-docs/blob/main/articles/17.Before.md).
...
`after` is also available at the **iteration** level (runs once when the iteration completes). See the [before & after Expressions article](https://github.com/mohaidr/lps-docs/blob/main/articles/17.Before.md).
